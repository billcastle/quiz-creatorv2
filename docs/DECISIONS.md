...
## ADR-011 — Biome as the single formatter + linter (replaces ESLint + Prettier) (2026-07-02)
**Status:** accepted
**Context:** The ROADMAP Phase 0 tooling step historically read "shared lint/format config (ESLint + Prettier)". During TICKET-001/002 the tooling decision landed on **Biome** as a single tool covering both formatting and linting (recorded in TICKET-001 notes, implemented in TICKET-002). This ADR reconciles the docs with what was actually built.
**Decision:** Use **Biome** (pinned `@biomejs/biome` 1.9.4, 1.x schema) as the one repo-root formatter + linter. `biome.json` enables the formatter, linter (recommended rule set), and `organizeImports`, with formatter settings matching `.editorconfig` (2-space indent, LF). Do NOT install ESLint or Prettier. Root scripts: `lint` = `biome check .`, `format` = `biome format --write .` (plus `format:check`). TypeScript type-checking stays separate via `tsc --noEmit` (`typecheck` script). `.editorconfig` remains for editor UX only; Biome is the enforcer.
**Consequences:** One fast tool instead of two toolchains (no ESLint/Prettier config drift, fewer devDeps). Rule coverage is Biome's, which is broad but not identical to the ESLint plugin ecosystem — acceptable for MVP; specific rules can be tuned in `biome.json` later. ROADMAP Phase 0 updated to say Biome (see ROADMAP reconciliation note).
